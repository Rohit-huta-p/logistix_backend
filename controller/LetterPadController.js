...
const getLetterPads =async (req, res) => {
    ...
    // if(userFound){
        ...
        if (letterPadFetched)
            return res
            .status(200)
            .json({ success: true, message: "All Letter Pad fetched", content: letterPadFetched });
...
const addLetterPad = async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({success:true, message: "Letter Pad ADDED", content: letterPad_added });
    } catch (error) {
